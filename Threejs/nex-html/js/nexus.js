...
function getUint16(view) {
	var s = view.getUint16(view.offset, true);  //低位优先顺序存取数据
	view.offset += 2;
	return s;
}
...
function matInv(m, t) {   //单位化?
	var s = 1.0/(   //好像是计算的行列式的值的倒数
...
//					console.log("200 response: server does not support byte range requests.");
	...
	handleIndex: function(buffer) {  //解析从硬盘读来的二进制文件
		var t = this;
		var view = new DataView(buffer); //本质是一个ArrayBuffer(类型化数组的子视图),。
		view.offset = 0;

		var n = t.nodesCount;

		t.noffsets  = new Uint32Array(n); //偏移位置
		t.nvertices = new Uint32Array(n); //定点数量?
		t.nfaces    = new Uint32Array(n); //面片数?
		...
		for(j = 0; j < t.nroots; j++) {  //该循环貌似没啥影响,作用是啥?
		...
		for(i = 0; i < t.texturesCount; i++) {
			...
			view.offset += 16*4; //skip proj matrix  //32位是4字节,投影矩阵有16个数字,所以跳过16*4
		}

		t.vsize = 12 + (t.vertex.normal?6:0) + (t.vertex.color?4:0) + (t.vertex.texCoord?8:0);  //定点shader?
		t.fsize = 6;                                                                            //片段shader?
		...
		t.ibo    = new Array(n);
		t.vbo    = new Array(n);  //vbo buffer
		t.texids = new Array(n); //texture索引
...
Instance.prototype = {
	...
	updateView: function(viewport, projection, modelView) {
		...
		matInv(t.modelView, t.modelViewInv);  //矩阵求逆 .所得结果应该是相机在世界坐标系的位置
	...
	traversal : function () {
		...
		t.visitQueue = new PriorityQueue(n);  //error data size
		 for(var i = 0; i < t.mesh.nroots; i++)   //添加根节点显示　．无论如何都会显示第一层节点
		   	t.insertNode(i);
       /*
         for(var i = t.N; i < t.M; i++)  //加载指定节点
             t.insertNode(i);
		*/
		...
		while(t.visitQueue.size && nblocked < maxBlocked) {
			...
			var node = t.visitQueue.pop();    //获取优先队列节点索引
			if ((requested < maxPending) && (t.mesh.status[node] == 0)) {　　//判断是否添加候选节点
...
//			console.log("Node " + node + " still not loaded (cache?)");
	...
	nodeError : function (n, tight) {
		...
		var b = t.viewpoint;   //相机在世界坐标的位置
		var off = n*5;
		var cx = spheres[off+0];　//获取该节点的包围球信息　
		...
		var d1 = b[1] - cy;
		var d2 = b[2] - cz;
		var dist = Math.sqrt(d0*d0 + d1*d1 + d2*d2) - r;  //计算相机到该节点包围球球心的距离与球半径的差值
		if (dist < 0.1)　//判断相机是否在几点包围球内部
			dist = 0.1;
	...
	isVisible : function (x, y, z, r) {
		var p = this.planes;  //猜测:这个平面数组不会是相机的视锥吧. 亦或者是整个模型的包围盒的六个面
		for (i = 0; i < 24; i +=4) {  //判断节点的球心是否在保卫盒内/外
			if(p[i]*x + p[i+1]*y + p[i+2]*z +p[i+3] + r < 0) //ax+by+cz+w = 0;  沿法线方向在平面外
				return false;
		}
		return true;
	},
	...
	renderNodes: function() {
		...
		for(var n = 0; n < m.nodesCount; n++) {
			...
			var nv = m.nvertices[n]; //是否对应纹理映射坐标?

            //将绑定到array_buffer的对象分配给attribute变量.3指定该position有3个分量,12指定相邻两个属性之间的步长(其实就是该属性的字节长度),false指定不归一化,0指定从缓冲区0字节开始
			gl.vertexAttribPointer(attr.position, 3, gl.FLOAT, false, 12, 0);  //将缓冲区对象分配给attribute变量.12指定相邻两个点之间的字节数
			var offset = nv*12;
...
//				gl.vertexAttrib4fv(2, [(n*200 %255)/255.0, (n*140 %255)/255.0,(n*90 %255)/255.0, 1]);
			...
			for (var p = m.nfirstpatch[n]; p < m.nfirstpatch[n+1]; ++p) {
				...
				if(end > offset) {
                   ...
                   // gl.drawElements(gl.LINE_STRIP, (end - offset) * 3, gl.UNSIGNED_SHORT, offset * 6); //三角形线段显示
					rendered += end - offset;
				}
...
function loadNodeTexture(request, context, node, texid) {
	...
	img.onload = function() { 
		...
		gl.bindTexture(gl.TEXTURE_2D, tex);
		var s = gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, img);  //绑定纹理到GPU
...
function readyNode(node) {
	...
	if(!m.corto) {
        vertices = new Uint8Array(node.buffer, 0, nv*m.vsize);  //真正的顶点数据。但为啥是uint类型？
		//dangerous alignment 16 bits if we had 1b attribute
		indices  = new Uint8Array(node.buffer, nv*m.vsize,  nf*m.fsize); //真正的索引
		if(nf == 0) {
			var off = nv*12;
	...
	gl.bufferData(gl.ARRAY_BUFFER, vertices, gl.STATIC_DRAW);  //将顶点数据写入到缓冲区(vbo中)
	var ibo = m.ibo[n] = gl.createBuffer();
	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, ibo);               //将索引写入到索引缓冲区
